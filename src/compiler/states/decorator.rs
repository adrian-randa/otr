use crate::{
    compiler::{
        CompilerEnvironment, CompilerError, CompilerState, states::{module::CompilerModuleState, procedure::CompilerProcedureState}
    }, core::CompiledObject, lexer::token::{KeywordToken, PunctuationToken, Token}
};
        ...
        match token {
            ...
            Token::Identifier(ref ident) => {
                if self.decorators.len() >= self.num_decorators {
                    ...
                    self.decorators.push(RawDecorator {
            ...
            Token::Keyword(KeywordToken::Proc) => {
                return Ok(Box::new(CompilerProcedureState::new(
                    self.module,
                    self.decorators,
                )));
            }
